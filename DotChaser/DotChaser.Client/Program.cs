...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        var player = new Player(new Vector2(3, 2), new Vector2(0, 0), 1000);
        Game game = new Game([player], new ElaborateMap(), new ClientOutputProvider());
        
        while (true)
        {
            if(DateTime.Now - lastSimulationStep > TimeSpan.FromSeconds(0.5))
            {
